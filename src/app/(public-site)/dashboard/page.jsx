...
<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    { businesses.length === 0 && (
       <button
          onClick={() => router.push("/business/register")}  className="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-500">
          List My Business
       </button>
    )}
      </div>
 ...
 <p className="text-sm mt-2">
  ...
  <p className="text-sm mt-2">
    <strong>End Date:</strong> {p.EndDate ? formatDate(p.EndDate) : "N/A"}
                             ...
                             {activeTab === "inProgress" && p.Status === String(p.Status) && b.Status === 3 && (
                        <button
