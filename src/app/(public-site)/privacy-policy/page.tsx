...
export default function PrivacyPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        // Fetch content for 'privacy' page
        const data = await getCmsContent("PrivacyPolicy", lang);
        setCmsEntry(data);
      } catch {
        // Set a localized fallback message on API failure
        setCmsEntry({
          CmsPageName: "Privacy Policy",
          CmsText: t('privacyPage.contentNotFound')
        });
      }
    }
    fetchData();
    ...
    <>
      ...
      <section className="bg-gray-100 text-gray-800 py-11">
        <div className="container mx-auto text-center px-4">
          <h1 className="text-4xl font-bold mb-4">{t('pageTitles.privacyTitle')}</h1>
            ...
            {cmsEntry ? (
              ...
              <div className="description text-center py-10 text-gray-600">
                <p className="text-xl font-medium">
                  {t('privacyPage.loading', { language: lang.toUpperCase() })}
                </p>
              </div>
            )}
